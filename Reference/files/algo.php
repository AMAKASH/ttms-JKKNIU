...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT * FROM teachers WHERE Teacher's_ID='$temp->subjectteacher'");
...
period7='" . $subjectslots[$i][$j][6] . "<br>" . $aliasslots[$i][$j][6][0] . ", " . $aliasslots[$i][$j][6][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
